...
import { defaultTextColor, innerMargin, Separator, defaultFontSize } from "./Styles"
...
const ContactLine = ({ url, logo, children }) =>
    url ?
        <Link src={url} style={styles.contactLine}>
            {logo}
...
const Contact = ({ title, githubPseudo, phone, email }) => <View style={styles.card}>
    <CategoryTitle>{title}</CategoryTitle>
    <View style={styles.inner}>
        <ContactLine url={`https://github.com/${githubPseudo}`} logo={githubLogo}>{githubPseudo}</ContactLine>
        <Separator verticalMargin={8} />
        <ContactLine url={`tel:${phone.replace(/\s+/g, '')}`} logo={phoneLogo}>{phone}</ContactLine>
        <Separator verticalMargin={8} />
        <ContactLine url={`mailto:${email}`} logo={emailLogo}>{email}</ContactLine>
    </View>
</View>
